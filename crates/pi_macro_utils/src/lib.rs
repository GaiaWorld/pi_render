...
impl PiManifest {
    pub fn maybe_get_path(&self, name: &str) -> Option<syn::Path> {
        ...
        const PI_INTERNAL: &str = "pi_render";
        ...
        let find_in_deps = |deps: &DepsSet| -> Option<syn::Path> {
            ...
            } else if let Some(dep) = deps.get(PI_INTERNAL) {
                dep.package().unwrap_or(PI_INTERNAL)
            } else {
                return None;
            };

            let mut path = Self::parse_str::<syn::Path>(package);
            if let Some(module) = name.strip_prefix("pi_") {
