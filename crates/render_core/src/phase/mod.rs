...
pub fn sort_phase_system<Archetype: 'static + Sync + Send, I: PhaseItem>(
    mut render_phases: Query<Archetype, &mut RenderPhase<I>>,
) {
...
pub fn batch_phase_system<Archetype: 'static + Sync + Send, I: BatchedPhaseItem>(
    mut render_phases: Query<Archetype, &mut RenderPhase<I>>,
) {
    for mut phase in render_phases.iter_mut() {
        phase.batch();
