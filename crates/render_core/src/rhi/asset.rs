use wgpu::{util::DeviceExt, AstcBlock, AstcChannel, TextureView};
use derive_deref_rs::Deref;
use pi_assets::{
    asset::{Asset, Garbageer, Handle, Size},
    mgr::{LoadResult, Receiver},
};
use pi_atom::Atom;
use pi_futures::BoxFuture;
use pi_hal::{image::DynamicImage, loader::AsyncLoader};

use super::{device::RenderDevice, texture::PiRenderDefault, RenderQueue};
...
impl<'a, G: Garbageer<Self>> AsyncLoader<'a, Self, ImageTextureDesc<'a>, G> for TextureRes {
    fn async_load(
        desc: ImageTextureDesc<'a>,
        result: LoadResult<'a, Self, G>,
    ) -> BoxFuture<'a, std::io::Result<Handle<Self>>> {
        Box::pin(async move {
            match result {
                LoadResult::Ok(r) => Ok(r),
                LoadResult::Wait(f) => f.await,
    ...
    let (width, height, buffer, format, pre_pixel_size, is_opacity) = match image {
        ...
        DynamicImage::ImageRgb8(r) => {
            ...
            (
                ...
                r.height(),
                buffer_temp.as_raw(),
                if <wgpu::TextureFormat as PiRenderDefault>::is_srgb() {
