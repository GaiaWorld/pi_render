...
use render_crevice::std140::{Writer, AsStd140, DynamicUniform};
use wgpu::{BindingResource, BufferBinding, BufferDescriptor, BufferUsages};
use super::{RenderDevice, RenderQueue, buffer::Buffer};
...
impl<T: AsStd140> UniformVec<T> {
    ...
    pub fn write_buffer(&mut self, device: &RenderDevice, queue: &RenderQueue) {
        ...
        if let Some(uniform_buffer) = &self.uniform_buffer {
            ...
            let mut writer = Writer::new(&mut self.scratch[range.clone()]);
