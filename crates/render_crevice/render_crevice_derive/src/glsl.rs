...
pub fn emit(input: DeriveInput) -> TokenStream {
    ...
    let base_trait_path: Path = parse_quote!(#render_crevice_path::glsl::Glsl);
    let struct_trait_path: Path = parse_quote!(#render_crevice_path::glsl::GlslStruct);
    ...
    let glsl_fields = fields.named.iter().map(|field| {
        ...
        let field_as = quote! {<#field_ty as #render_crevice_path::glsl::GlslArray>};
        ...
        quote! {
            ...
            <#field_as::ArraySize as #render_crevice_path::glsl::DimensionList>::push_to_string(s);
            s.push_str(";\n");
        }
    });
