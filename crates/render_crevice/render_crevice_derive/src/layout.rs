...
pub fn emit(
    ...
    let render_crevice_path = crate::render_crevice_path();

    let mod_name = Ident::new(mod_name, Span::call_site());
    let trait_name = Ident::new(trait_name, Span::call_site());

    let mod_path: Path = parse_quote!(#render_crevice_path::#mod_name);
    ...
    let struct_alignment = quote! {
        #render_crevice_path::internal::max_arr([
        ...
        .map(|(index, prev_field)| {
            ...
            quote! {
                ...
                const fn #pad_fn() -> usize {
                    ...
                    let alignment = #render_crevice_path::internal::max(
                        #next_field_or_self_alignment,
                        min_alignment,
                    );

                    // Using everything we've got, compute our padding amount.
                    #render_crevice_path::internal::align_offset(starting_offset, alignment)
                }
            }
        })
        .collect();
        ...
        quote! {
            impl #impl_generics #generated_name #ty_generics #where_clause {
                fn debug_metrics() -> String {
                    ...
                    let zeroed: Self = #render_crevice_path::internal::bytemuck::Zeroable::zeroed();
    ...
    quote! {
        ...
        unsafe impl #impl_generics #render_crevice_path::internal::bytemuck::Zeroable for #generated_name #ty_generics #where_clause {}
        unsafe impl #impl_generics #render_crevice_path::internal::bytemuck::Pod for #generated_name #ty_generics #where_clause {}
        ...
        unsafe impl #impl_generics #mod_path::#trait_name for #generated_name #ty_generics #where_clause {
            ...
            type Padded = #padded_path<Self, {#render_crevice_path::internal::align_offset(
        ...
        impl #impl_generics #as_trait_path for #input_name #ty_generics #where_clause {
            ...
            fn #as_trait_method(&self) -> Self::Output {
                Self::Output {
                    ...
                    ..#render_crevice_path::internal::bytemuck::Zeroable::zeroed()
                }
            }
